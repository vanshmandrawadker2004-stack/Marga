...
function parseConversationalQuery(rawQuery) {
    ...
    // vague/emotional filler
    'nothing','too','crazy','just','wanna','wanna','gonna','need','really','very','so','get','lost','clear','head','alone','different','never','been','tired','again','not','no','without','but','and','or','if','then','also','only','always','never','ever','maybe','probably','simply','basically','actually','literally','honestly','tbh','ngl','idk','lol','feels','feel','feeling','mood','vibe','vibes','kinda','sorta','super','totally','man','bro','dude','yaar','toh','ahe','jaaycha','kahi','tari','chan','thoda','duur','mala','ek'];
...
document.querySelectorAll('.utility-toggle').forEach(toggle => {
    toggle.addEventListener('change', (e) => {
        ...
        const overpassData = `[out:json];${query}out;`;
        fetch(`/api/overpass?query=${encodeURIComponent(overpassData)}`)
            .then(r => r.ok ? r : fetch(`https://maps.mail.ru/osm/tools/overpass/api/interpreter?data=${encodeURIComponent(overpassData)}`))
            .then(res => res.json())
            .then(data => {
                if (label) label.style.opacity = '';
